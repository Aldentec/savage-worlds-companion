...
const generateAttributes = () => {
    ...
    if (!uniqueDice.has("d4")) attributes[getRandomAttribute(attributes)] = "d4";
...
const generateSkills = () => {
    const defaultSkills = skills.map(skill => {
      ...
      const diceTypeOptions = ["d4", "d6", "d8", "d10"];
...
const generateGear = () => {
  const selectedWeapons = [generateRandomValue(bossGearOptions.weapons)];
  const selectedArmor = [generateRandomValue(bossGearOptions.armor)];
  const selectedItems = [generateRandomValue(bossGearOptions.items)];
  return {
...
export default generateBossNPC;
